...
from kivy.graphics import Rectangle, Color, Ellipse, PushMatrix, PopMatrix, RoundedRectangle
from kivy.uix.button import Button
import random
...
class AnimatedPieceButton(ButtonBehavior, Widget):
    selected = False
    ...
    def __init__(self, **kwargs):
        ...
        self.background_color = kwargs.get('background_color', (0.4, 0.7, 0.4, 1))
    ...
    def select(self):
        self.selected = True
        anim = Animation(background_color=(0.8, 0.8, 1, 1), duration=0.2)
        anim.start(self)
...
class DominoGameGUI(BoxLayout):
    ...
    def setup_gui(self):
        # Board display
        self.board_layout = BoxLayout(orientation='vertical', size_hint_y=0.3, padding=10)
    ...
    def update_display(self):
        ...
        for i, piece in enumerate(self.players[0]):
            btn = AnimatedPieceButton(
                text=str(piece),
                background_color=(0.4, 0.7, 0.4, 1) if self.current_player == 0 else (0.5, 0.5, 0.5, 1)
            )
            btn.piece_index = i
    ...
    def play_domino(self, *args):  # Add *args parameter
        ...
        if not self.game_active:
            self.show_animatedpopup("Game Over", "This game has ended. Please start a new game.")
            return False
    ...
    def show_animated_popup(self, title, message):
        ...
        def dismiss_with_animation(*args):
            anim = Animation(opacity=0, duration=0.2)
            anim.bind(on_complete=lambda *x: popup.dismiss())
            anim.start(popup)
            
        dismiss_button.bind(on_press=dismiss_with_animation)
    ...
    def handle_ai_turn(self):
        ...
        if move_index is not None:
            ...
            self.status_bar.text = f"{self.player_names[self.current_player]} is passing"  # Changed from config
            self.handle_pass()
...
class DominoApp(App):
    ...
    def on_start(self):
        # Set custom colors for popups
        self.root.settings_popup = None
