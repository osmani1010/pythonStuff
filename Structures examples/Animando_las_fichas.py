...
class DominoPiece(Button):
    ...
    def __init__(self, domino, **kwargs):
        ...
        self.size = (60, 30)
        self.text = str(domino)
        self.background_normal = ''
        self.background_color = (0.9, 0.9, 0.9, 1)
        self.color = (0, 0, 0, 1)

        # self.scale = 1.0  # Add scale property
        
        with self.canvas.before:
            PushMatrix()
            self.rotation_instruction = Rotate(angle=0, origin=self.center)
            self.scale_instruction = Scale(1, 1, 1)
            Color(0.9, 0.9, 0.9, 1)
            self.rect = Rectangle(pos=self.pos, size=self.size)
            Color(0, 0, 0, 1)
            middle_x = self.x + self.width / 2
            self.line = Line(points=[middle_x, self.y, middle_x, self.y + self.height])
            PopMatrix()

        
        self.bind(pos=self.update_rect, size=self.update_rect, scale=self.update_scale, rotation=self.update_rotation)

    def update_rotation(self, instance, value):
        self.rotation_instruction.angle = value

    def update_scale(self, instance, value):
        self.scale_instruction.xyz = (value, value, 1)  
    ...
    def update_rect(self, *args):
        ...
        middle_x = self.x + self.width / 2
        self.line.points = [middle_x, self.y, middle_x, self.y + self.height]

    def flip_piece(self):
        # Add rotation animation when piece is flipped
        anim = Animation(rotation=360, duration=0.3)
        anim.start(self)    
...
class BoardLayout(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.center_layout = BoxLayout(orientation='horizontal', spacing=5, size_hint_x=None)
    ...
    #     # Use available width (considering some padding)
    #     available_width = 0.8  # Use 80% of the width
    #     start_x = (1 - available_width) / 2  # Start from 10% of the width
        
    #     # Calculate spacing between pieces
    #     if total_pieces > 1:
    #         spacing = available_width / (total_pieces - 1)
    #         # Limit minimum spacing
    #         spacing = min(spacing, 0.15)  # Maximum spacing of 15% of width
    #         # Recalculate start_x to center the pieces
    #         total_width = spacing * (total_pieces - 1)
    #         start_x = (1 - total_width) / 2
    #     else:
    #         # Single piece should be centered
    #         return [{'center_x': 1, 'center_y': 0.5}]
...
class DominoGameGUI(BoxLayout):
    ...
    def setup_gui(self):
        ...
        self.pieces_grid = GridLayout(
            ...
            row_default_height=40,  # Default height for rows
            col_default_width=70,  # Default width for columns
            col_force_default=True,  # Force the column width
            row_force_default=True   # Force the row height
        )
        
        hand_container = BoxLayout(orientation='vertical', size_hint_y=0.4)
        hand_container.add_widget(self.hand_label)
        hand_container.add_widget(self.pieces_grid)
        ...
        self.exit_button = Button(text="Exit Game", on_press=self.confirm_exit)
    ...
    def update_display(self):
        ...
        for i, piece in enumerate(self.players[0]):
            piece_widget = DominoPiece(
                ...
                size_hint=(None, None),
                pos_hint={},
                background_color=(0.3, 0.8, 0.3, 1) if self.current_player == 0 
                else (0.7, 0.7, 0.7, 1)
            ...
            if i == self.selected_piece_index and self.current_player == 0:
                piece_widget.background_color = (0.8, 0.8, 1, 1)
    ...
    def _try_play_piece(self, piece, piece_index, first_domino, last_domino):
        ...
        def play_with_flip(flip_needed, position):
            if flip_needed:
                # domino_widget = self.board_layout.pieces[-1]  # Get the last added piece
                # # Create flip animation
                # anim = Animation(rotation=180, duration=0.3)
